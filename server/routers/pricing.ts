...
async function detectMarketChanges(
  ...
  for (const [hotelId, currentPrices] of Object.entries(currentByHotel)) {
    const prevPrices = (previousByHotel as Record<string, typeof currentPrices>)[hotelId] || [];
    
    if (prevPrices.length === 0) continue;

    const avgCurrent = currentPrices.reduce((sum: number, p: any) => sum + p.price, 0) / currentPrices.length;
    const avgPrevious = prevPrices.reduce((sum: number, p: any) => sum + p.price, 0) / prevPrices.length;
